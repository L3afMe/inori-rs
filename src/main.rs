...
#[strikethrough_commands_tip_in_dm(
    "Commands with a ~~`strikethrough`~~ require \
    certain lacking permissions to execute."
)]
...
static NITRO_REGEX: Lazy<Regex> = Lazy::new(|| {
    Regex::new("(discord.com/gifts/|discordapp.com/gifts/|discord.gift/)[ ]*([a-zA-Z0-9]{16,24})")
        .unwrap()
});
...
async fn normal_message(ctx: &Context, msg: &Message) {
            ...
            match res.status() {
                StatusCode::OK => {
                    if msg.is_private() {
                        ...
                        let channel_name =
                            match ctx.http.get_channel(msg.channel_id.0).await.unwrap() {
    ...
    if msg.author.id.0 == 346353957029019648
        && msg.content.starts_with(
            "Someone just dropped their wallet in this channel! Hurry and pick it up with",
        )
    {
        let config = {
            ...
            .eq("<:yay:585696613507399692>   **GIVEAWAY**   <:yay:585696613507399692>")
    {
        ...
        let guild_name = msg
            .guild_id
            .unwrap() // Check if it's a guild above so this will never throw error
            .name(&ctx.cache)
            .await
            .unwrap_or("Unknown".to_string());
...
async fn dispatch_error(ctx: &Context, msg: &Message, error: DispatchError) {
    ctx.http
        .delete_message(msg.channel_id.0, msg.id.0)
        .await
        .unwrap();
            ...
            match reason {
                Reason::User(err) => {
                    let content = match err.as_ref() {
                        "nsfw_1" => {
                            "This server is not marked as NSFW and you've specified a NSFW image.\n\
                        This can be overriden by executing `nsfwfilter 1`"
                                .to_string()
                        }
                        "nsfw_2" => {
                            "This server is not marked as NSFW and you've specified a NSFW image.\n\
                        This can be overriden by executing `nsfwfilter 2`"
                                .to_string()
                        }
                        _ => {
                            format!(
            ...
            loop {
                {
                    ...
                    if settings.pfp_switcher.enabled
                        && start_time.elapsed().unwrap().as_secs()
                            >= (settings.pfp_switcher.delay * 60) as u64
                    {
                        ...
                        let avatar = read_image(format!(
                            "./pfps/{}",
                            new_pfp.file_name().into_string().unwrap()
                        ))
                        .unwrap();

                        user.edit(&ctx.http, |p| p.avatar(Some(&avatar)))
                            .await
                            .unwrap();
