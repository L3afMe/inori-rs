...
async fn remove(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    if !settings.giveaway.blacklisted_guilds.contains(&guild_id) {
        drop(settings);
        drop(data);
        ...
        return msg
            .channel_id
            .send_tmp(ctx, |m: &mut MessageCreator| {
                m.info().title("Giveaway").content("Guild not currently blacklisted")
            })
            .await;
    } else {
        let filtered = settings
            ...
            .filter(|g| *g != guild_id)
            .collect::<Vec<u64>>();
        settings.giveaway.blacklisted_guilds = filtered;
        ...
        return msg
            .channel_id
            .send_tmp(ctx, |m: &mut MessageCreator| {
                m.success().title("Giveaway").content("Removed guild from blacklist")
            })
            .await;
    };
}
...
async fn add(ctx: &Context, msg: &Message, mut args: Args) -> CommandResult {
    ...
    if settings.giveaway.blacklisted_guilds.contains(&guild_id) {
        ...
        return msg
            .channel_id
            .send_tmp(ctx, |m: &mut MessageCreator| {
                m.info().title("Giveaway").content("Guild already blacklisted")
            })
            .await;
    } else {
        settings.giveaway.blacklisted_guilds.push(guild_id);
        save_settings(&settings);
        ...
        return msg
            .channel_id
            .send_tmp(ctx, |m: &mut MessageCreator| {
                m.success().title("Giveaway").content("Added guild to blacklist")
            })
            .await;
    };
}
